...
const schema = yup.object({
    ...
    password: yup.string().min(6, 'Password must be minimum 6 characters').required(),
    confirmPassword: yup.string()
        .oneOf([yup.ref('password')], 'Passwords must match')
        .required('Please confirm your password'),
    role: yup.string().default('tenant'),
}).required();
...
const TenantRegisterPage: React.FC = () => {
    ...
    const onSubmit = async (data: any) => {
        ...
        if (result && result.success) {
            ...
            dispatch(setRegistrationData({ email: data.email, role: 'tenant', phone: data.phone }));
            navigate('/tenant/verify-otp', { state: { email: data.email, expiresAt: result.expiresAt } });
        } else {

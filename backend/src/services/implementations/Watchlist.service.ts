...
export class WatchListService implements IWatchListService {
    ...
    async findAllWatchListItems(page: number, limit: number, userId: string): Promise<IGenericPaginatedResposnse<WatchlistItemCardDTO>> {
        const { items, total } = await this._watchlistRepo.findAllWatchListItems(page, limit, userId);
        return {
            data: items,
            pagination: {
                totalItems: total,

...
export class ProfileService implements IProfileService {
    ...
    async changeEmail(data: changeEmailDTO): Promise<changeEmailResponseDto> {
        if (data.newEmail === data.oldEmail) {
            throw new ValidationError('Old email and new Email Cannot same')
        }
    ...
    async changeEmailResendOtp(data: ResendOtpDTO): Promise<changeEmailResponseDto> {
        const otpresult = await this._otpService.resendOtp(data.email, CONFIG.OTP_EXPIRY_MINUTES, CONFIG.OTP_RESEND_DELAY_SECONDS)
